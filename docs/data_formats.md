...
The `complete_parts_viewer.html` file was generated by a **Unreal DataTable export tool** (likely FModel, UAssetGUI, or similar). These tools can:
...
### Method 1: FModel (Recommended for full data)

[FModel](https://fmodel.app/) can parse UE5 IoStore format and extract DataTables with resolved strings.

1. Download FModel
2. Configure for BL4's AES key (if encrypted)
3. Open `.pak` or `.utoc` files
4. Navigate to DataTable assets
5. Export as JSON or view in UI

**Note**: FModel has better UE5.5 support than retoc. The `complete_parts_viewer.html` was likely generated using FModel or a similar tool.

### Method 2: UAssetGUI (Legacy format only)
