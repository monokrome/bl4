...
impl SaveFile {
    ...
    /// Exact XP threshold for a character level from manifest `experience_progression.tsv`
    ///
    /// Pulled at build time via `include_str!(concat!(env!("OUT_DIR"), "/experience_progression.tsv"))`
    /// (copied by `build.rs` from `share/manifest/experience_progression.tsv`, which is
    /// generated by `ncs extract --extract-type xp-progression`). Source NCS is
    /// `Oak2_CharacterXP_Progression` `GbxExperienceFunction_Exponential` `60*L^2.8`.
    /// Verified: `60:5714454` vs save est `5714893` (+439), `50:3429787` vs `3430207` (+420).
    pub fn xp_for_character_level(level: u64) -> u64 {
        const TSV: &str = include_str!(concat!(env!("OUT_DIR"), "/experience_progression.tsv"));
        // Lazy parse on first call — TSV is small (400 rows), so scanning is fine.
        // Expected columns: progression, level, xp, ... ; filter for oak2_characterxp_progression
        for line in TSV.lines().skip(1) {
            let mut cols = line.split('\t');
            let prog = cols.next().unwrap_or("");
            if prog.to_lowercase() != "oak2_characterxp_progression" {
                continue;
            }
            ...
            if lvl == level {
                if let Some(xp_str) = cols.next() {
                    if let Ok(xp) = xp_str.parse::<u64>() {
                        // NCS has 60 for L=1, but saves start at 0 for L=1
                        if level == 1 {
                            return 0;
                        }
                        return xp;
        ...
        // Fallback for levels not in TSV (shouldn't happen for 1..100)
        0
    }
