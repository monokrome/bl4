...
mod tests {
    ...
    #[test]
    #[ignore]
    fn test_reverse_engineer_binary_format() {
        use crate::parser::{parse_header, find_binary_section_with_count};
        use crate::string_table::parse_string_table;
        use serde_json::Value;

        let inv_path = "/home/polar/Documents/Borderlands 4/ncsdata/pakchunk4-Windows_0_P/Nexus-Data-inv4.bin";
        let json_path = "/home/polar/Documents/Borderlands 4/ncsp/json/Nexus-Data-inv4.json";
        ...
        // Get first few serial indices from JSON
        let records = json["inv"]["records"].as_array().expect("No records");
        let first_record = &records[0];
        let entries = first_record["entries"].as_array().expect("No entries");
        let first_entry = &entries[0];

        println!("First entry in JSON:");
        for (key, value) in first_entry.as_object().expect("Not object") {
            if key == "__dep_entries" {
                continue;
            }

            if let Some(si) = value.get("serialindex") {
                let index = si["index"].as_str().unwrap().parse::<u32>().unwrap();
                println!("  Item '{}': serial index = {}", key, index);

                // Search for this value in entire binary section
                println!("    Searching for {} (0x{:x}) in binary...", index, index);
                let mut found_count = 0;
                for i in 0..binary_data.len().min(10000) {
                    if binary_data[i] as u32 == index {
                        println!("      Found as byte at offset 0x{:x}", i);
                        found_count += 1;
                        if found_count > 5 { break; }
                    }
                    if i + 1 < binary_data.len() {
                        let val = u16::from_le_bytes([binary_data[i], binary_data[i+1]]) as u32;
                        if val == index {
                            println!("      Found as u16 LE at offset 0x{:x}", i);
                            found_count += 1;
                            if found_count > 5 { break; }
                        }
                    }
                ...
                if found_count == 0 {
                    println!("      Not found in first 10000 bytes");
                }
            }

            // Check first dep_entry
            if let Some(dep_entries) = value.get("__dep_entries").and_then(|v| v.as_array()) {
                if let Some(first_dep) = dep_entries.get(0) {
                    let dep_obj = first_dep.as_object().expect("Not object");
                    println!("  First dep_entry:");
                    for (part_key, part_value) in dep_obj {
                        if part_key.starts_with("dep") {
                            continue;
                        }
                        if let Some(si) = part_value.get("serialindex") {
                            let index = si["index"].as_str().unwrap().parse::<u32>().unwrap();
                            println!("    Part '{}': serial index = {}", part_key, index);
        ...
        // Collect first 30 serial indices in order from Record 0
        println!("\n\nCollecting sequence of serial indices from JSON:");
        let mut serial_sequence = Vec::new();

        // Go through first entry and collect all serial indices
        for entry in entries.iter().take(1) {
            let entry_obj = entry.as_object().expect("Not object");

            // Get top-level item serial index (e.g., armor_shield)
            for (key, value) in entry_obj {
                ...
                if let Some(si) = value.get("serialindex") {
                    let index = si["index"].as_str().unwrap().parse::<u32>().unwrap();
                    serial_sequence.push(index);
                }
            }

            // Get dep_entry serial indices
            if let Some(dep_entries) = entry_obj.get("__dep_entries").and_then(|v| v.as_array()) {
                for dep in dep_entries.iter() {
                    let dep_obj = dep.as_object().expect("Not object");
                    for (part_key, part_value) in dep_obj {
                        if part_key.starts_with("dep") {
                            continue;
                        }
        ...
        println!("First {} serial indices:", serial_sequence.len());
        for (i, &idx) in serial_sequence.iter().enumerate() {
            print!("{} ", idx);
            if (i + 1) % 10 == 0 {
                println!();
            }
        ...
        // Search for first 20 serial indices and analyze patterns
        println!("Analyzing tag patterns around serial indices:");
        for &target_idx in serial_sequence.iter().take(20) {
            println!("\nSearching for serial index {} (0x{:x}):", target_idx, target_idx);

            // Search in first 20KB of binary
            let mut found_at = Vec::new();
            for i in 0..binary_data.len().min(20000) {
                // Try as single byte
                if binary_data[i] as u32 == target_idx {
                    found_at.push((i, "u8"));
                }
                // Try as u16 LE
                if i + 1 < binary_data.len() {
                    let val = u16::from_le_bytes([binary_data[i], binary_data[i + 1]]) as u32;
                    if val == target_idx {
                        found_at.push((i, "u16le"));
                    }
                }
            }

            if found_at.is_empty() {
                println!("  NOT FOUND in first 20KB");
            } else {
                println!("  Found at {} location(s)", found_at.len());
                for &(pos, encoding) in found_at.iter().take(3) {
                    println!("    Offset 0x{:x} (as {}):", pos, encoding);

                    // Show 40 bytes before and 10 after
                    if pos >= 40 {
                        print!("      Before: ");
                        ...
                        // Look for tag bytes in the 40 bytes before
                        ...
                        for i in (pos.saturating_sub(40))..pos {
                            if tag_bytes.contains(&binary_data[i]) {
                                tags_found.push((i, binary_data[i] as char, pos - i));
                            }
                        }

                        if !tags_found.is_empty() {
                            println!("      Tags found:");
                            for (tag_pos, tag_char, distance) in tags_found {
                                println!("        '{}' (0x{:02x}) at offset 0x{:x}, {} bytes before serial index",
                                    tag_char, tag_char as u8, tag_pos, distance);
                            }
                        }
                    }

                    print!("      Value: ");
                    for i in pos..pos.saturating_add(10).min(binary_data.len()) {
                        print!("{:02x} ", binary_data[i]);
                    }
                    println!();
                }
